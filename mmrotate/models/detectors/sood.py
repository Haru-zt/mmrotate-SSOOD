...
class SOOD(RotatedSemiBaseDetector):
    ...
    def __init__(
        ...
        self.reg_loss_type = self.semi_loss_cfg.get("reg_loss_type", "SmoothL1Loss")
        ...
        if self.aux_loss is not None:
            ...
            assert (
                self.aux_loss_cfg is not None
            ), "aux_loss_cfg should be provided when aux_loss is not None."
            self.ot_weight = self.aux_loss_cfg.pop("loss_weight", 1.0)
        ...
        assert self.dynamic_weight in ["None", "ang", "10ang", "50ang", "100ang"]

    @torch.no_grad()
    def get_pseudo_instances(
        self, batch_inputs: Tensor, batch_data_samples: SampleList
    ) -> Tuple[SampleList, Optional[dict]]:
        """Get dense pseudo instances from teacher model."""
        dense_predicts = self.teacher(batch_inputs)
        batch_info = {}
        batch_info["dense_predicts"] = dense_predicts

        """Get sparse pseudo instances from teacher model."""
        self.teacher.eval()
    ...
    def loss_by_pseudo_instances(
        ...
        for img_idx, data_samples in enumerate(batch_data_samples):
            if len(data_samples.gt_instances) > 0:
                ...
                pts = np.array(a)
                mask = np.zeros(
                    (batch_inputs.shape[-2], batch_inputs.shape[-1]), dtype=np.uint8
                )
        ...
        masks = masks.view(-1, 1, batch_inputs.shape[-2], batch_inputs.shape[-1])

        # interpolate the dense pseudo label area to FPN P5
        size_fpn_p5 = (int(batch_inputs.shape[-2] / 8), int(batch_inputs.shape[-1] / 8))

        masks = F.interpolate(masks.float(), size=size_fpn_p5).bool().squeeze(1)

        num_valid = (
            sum([_.sum() for _ in masks]) if isinstance(masks, list) else masks.sum()
        )

        if num_valid == 0:
            loss_cls = torch.tensor(0.0, device=gpu_device)
            loss_bbox = torch.tensor(0.0, device=gpu_device)
            loss_centerness = torch.tensor(0.0, device=gpu_device)
            if self.aux_loss is not None:
                loss_gc = torch.tensor(0.0, device=gpu_device)
                losses = {
                    "loss_cls": loss_cls,
                    "loss_bbox": loss_bbox,
            ...
            for i in range(len(masks)):
                if masks[i].sum() == 0:
                    continue
                teacher_cls_scores_logits_ = (teacher_cls_scores_logits[0][i]).permute(
                    1, 2, 0
                )[masks[i]]
                teacher_bbox_preds_ = (teacher_bbox_preds[0][i]).permute(1, 2, 0)[
                    masks[i]
                ]
                teacher_angle_pred_ = (teacher_angle_pred[0][i]).permute(1, 2, 0)[
                    masks[i]
                ]
                teacher_centernesses_ = (teacher_centernesses[0][i]).permute(1, 2, 0)[
                    masks[i]
                ]

                student_cls_scores_logits_ = (student_cls_scores_logits[0][i]).permute(
                    1, 2, 0
                )[masks[i]]
                student_bbox_preds_ = (student_bbox_preds[0][i]).permute(1, 2, 0)[
                    masks[i]
                ]
                student_angle_pred_ = (student_angle_pred[0][i]).permute(1, 2, 0)[
                    masks[i]
                ]
                student_centernesses_ = (student_centernesses[0][i]).permute(1, 2, 0)[
                    masks[i]
                ]
                ...
                with torch.no_grad():
                    if self.dynamic_weight in [
                        ...
                        loss_weight = (
                            torch.abs(
                                teacher_bbox_preds_[:, -1] - student_bbox_preds_[:, -1]
                            )
                            / np.pi
                        )
                        if self.dynamic_weight == "None":
                            loss_weight = torch.ones_like(loss_weight.unsqueeze(-1))
                        elif self.dynamic_weight == "ang":
                            loss_weight = (
                                torch.clamp(loss_weight.unsqueeze(-1), 0, 1) + 1
                            )
                        elif self.dynamic_weight == "10ang":
                            loss_weight = (
                                torch.clamp(10 * loss_weight.unsqueeze(-1), 0, 1) + 1
                            )
                        elif self.dynamic_weight == "50ang":
                            loss_weight = (
                                torch.clamp(50 * loss_weight.unsqueeze(-1), 0, 1) + 1
                            )
                        elif self.dynamic_weight == "100ang":
                            loss_weight = (
                                torch.clamp(100 * loss_weight.unsqueeze(-1), 0, 1) + 1
                            )
                        else:
                ...
                # bbox loss
                loss_bbox_ = (
                    self.loss_bbox(student_bbox_preds_, teacher_bbox_preds_)
        ...
        self.weight_suppress = self.semi_train_cfg.get("weight_suppress", "linear")
